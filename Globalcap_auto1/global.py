...
def unificar_archivos():
  ...
  try:
    ...
    # para no perder ni desfasar la primera semana
  ...
  # FUNCIÓN CLAVE AJUSTADA: Transforma Lunes en el Domingo anterior
  def encontrar_fecha_unificada(fecha_raw):
    try:
      ...
      # Si la fecha del Excel es un LUNES (dayofweek == 0), le restamos 1 día para ajustarla al DOMINGO
  ...
  # 2. PROCESAMIENTO DE ARCHIVOS EXCEL
  for file in archivos_subidos:
    try:
      ...
      # Detecta columnas de fechas (ej: 08-03, 08-10, 08/03/2026)
      columnas_fechas = [
          c
          for c in df.columns
          if c
